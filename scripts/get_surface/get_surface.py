...
COLOR = {1: "black", 2: "blue", 3: "green", 4: "magenta",
         5: "cyan", 6: "yellow", 7: "purple", 8: "lime", 9: "red"}
...
def send_image_to_clipboard(image: Image.Image):
    ...
    if not _WIN32_CLIPBOARD_AVAILABLE:
        print("⚠️  Копирование в буфер обмена поддерживается только на Windows (требуется pywin32).",
              file=sys.stderr)
        return False
    output = io.BytesIO()
...
def get_surface():
    ...
    if (Type == 4) or (Type >= 8):
        ...
        DORF  = np.array([0.7, 0.9, 1.0, 1.2, 1.4]) * np.deg2rad(DOR[4]) / 2  # ДОР ярусов
    ...
    def _eval_cr(expr, nr_arr):
        ...
        if isinstance(expr, str):
            ...
            ctx.update({
                "nr": nr_arr, "Ncr": Ncr, "H": H,
                "cos": _np.cos, "sin": _np.sin, "tan": _np.tan,
                "sqrt": _np.sqrt, "pi": _np.pi,
                "deg2rad": _np.deg2rad,
            })
            expr_py = expr.replace(".*", "*").replace("./", "/").replace(".^", "**").replace("^", "**")
            try:
    ...
    if state.test.SwT if hasattr(state.test, 'SwT') else True:
        if Tr.Pos.ndim == 2 and Tr.N > 0:
            m_idx = (np.round(np.linspace(0, Tr.N - 1, 11)).astype(int)
                     if Tr.N > 200 else np.arange(Tr.N))
        ...
        if St.N > 0 and St.Pos.ndim == 3:
            for cnt in range(St.N):
                ax.plot(St.Pos[m_idx, 0, cnt], St.Pos[m_idx, 2, cnt],
                        St.Pos[m_idx, 1, cnt], "-dr")
    ...
    if len(FC) > 0:
        ...
        if hasattr(Sf, 'TreeT') and Sf.TreeT == 1:
            ...
            cMass[1, FC] = dH[4] * (np.abs(np.random.rand(len(FC)) + 1j * np.random.rand(len(FC))) * 0.5 + 0.29289)
        else:
            # Многоярусный лес
            LevCount = np.random.randint(0, LevNum, size=len(FC))
            cMass[1, FC] = (cMass[1, FC] + np.random.rand(len(FC)) * LevdH[LevCount]
                            + LevH[LevCount + 1])                      # Y верх леса
            cMass[6, FC] = KrF[LevCount] * np.random.randn(len(FC))   # коэфф. отражения
            cMass[7, FC] = Sf.WindV * np.cosd(Sf.WindTh) * np.random.randn(len(FC))  # Vx по ветру
            cMass[8, FC] = Sf.WindV * np.sind(Sf.WindTh) * np.random.randn(len(FC))  # Vz по ветру
            cMass[9, FC] = DORF[LevCount]                              # ДОР ярусов

        ax.scatter(cMass[0, FC], cMass[2, FC], cMass[1, FC],
                   s=np.abs(cMass[6, FC] * cMass[9, FC] * Q + 10),
                   c=COLOR[4], marker="D", alpha=0.6)
    ...
    if len(FC) > 0:
        ...
        cMass[1, FC] = (np.abs(np.round(cMass[0, FC] * TownD * 0.01) / dH[7] + 0.5
                               - np.round(cMass[0, FC] * TownD * 0.01 / dH[7] + 0.5))
                        + np.abs(np.round(cMass[2, FC] / dH[7]) * 0.3333
        ...
        if len(FC) > 0:
            ...
            ax.stem(cMass[0, FC], cMass[2, FC], cMass[1, FC],
                    markerfmt=MARK[7], linefmt="m-", basefmt=" ")
    ...
    if len(FC) > 0:
        ...
        WaveL  = Sea.WaveL if Sea.WaveL > 0 else max(WindV ** 2 / 9.8, 1.0)  # длина волны по ветру
        Shift  = Sea.Shift if hasattr(Sea, 'Shift') else 0

        # Вектор движения волны вдоль направления ветра
        Vec = (cMass[0, FC] * np.sin(np.deg2rad(WindTh))
               - cMass[2, FC] * np.cos(np.deg2rad(WindTh)))
        ...
        cMass[10, FC] = np.sin(Arg) * np.sin(np.deg2rad(WindTh)) * dH[2] / WaveL     # наклон по X
        cMass[11, FC] = np.cos(Arg) * np.cos(np.deg2rad(WindTh)) * dH[2] / WaveL     # наклон по Z
        cMass[6, FC]  = Kr[2] * cMass[10, FC]                                         # коэфф. отражения

        ax.scatter(cMass[0, FC], cMass[2, FC], cMass[1, FC],
                   s=np.abs(cMass[6, FC]) * cMass[9, FC] * Q + 1,
                   c=COLOR[2], marker="o", alpha=0.6)
    ...
    if len(FC) > 0:
        ...
        WindV  = Sf.WindV if hasattr(Sf, 'WindV') else 0
        WindTh = Sf.WindTh if hasattr(Sf, 'WindTh') else 0
        cMass[1, FC] = cMass[1, FC] + (1 + 0.5 * np.random.randn(len(FC))) * dH[3]   # Y координата
        cMass[6, FC] = Kr[3] * np.random.randn(len(FC))                               # коэфф. отражения
        cMass[7, FC] = WindV * np.cos(np.deg2rad(WindTh)) * np.random.randn(len(FC))  # Vx травы
        cMass[8, FC] = WindV * np.sin(np.deg2rad(WindTh)) * np.random.randn(len(FC))  # Vz травы
        cMass[9, FC] = np.deg2rad(DOR[3]) / 2                                         # ДОР
        ax.scatter(cMass[0, FC], cMass[2, FC], cMass[1, FC],
                   s=5, c=COLOR[3], marker="+", alpha=0.7)
    ...
    if len(FC) > 0 and Type == 5:
        ...
        ax.scatter(cMass[0, FC1], cMass[2, FC1], cMass[1, FC1],
                   s=np.abs(cMass[6, FC1]) * cMass[9, FC1] * Q * 10,
                   c=COLOR[5], marker="^", alpha=0.7)
        ...
        ax.scatter(cMass[0, FC2], cMass[2, FC2], cMass[1, FC2],
                   s=np.abs(cMass[6, FC2]) * cMass[9, FC2] * Q,
                   c=COLOR[2], marker="o", alpha=0.5)
        legend_labels.append("Низ льда")
    elif len(FC) > 0:
        print(f"5mounts:{Sf.Types[5]}, ", end="")
        cMass[1, FC] = cMass[1, FC] + (1 + 0.5 * np.random.randn(len(FC))) * dH[5]   # Y координата
    ...
    if len(FC) > 0:
        ...
        cMass[1, FC] = cMass[1, FC] + (1 + 0.5 * np.random.randn(len(FC))) * dH[6]   # Y координата
        cMass[6, FC] = Kr[6] * np.random.randn(len(FC))                               # коэфф. отражения
        cMass[9, FC] = np.deg2rad(DOR[6]) / 2                                         # ДОР солончак
        ax.scatter(cMass[0, FC], cMass[2, FC], cMass[1, FC],
                   s=np.abs(cMass[6, FC]) * cMass[9, FC] * Q,
                   c=COLOR[6], marker="o", alpha=0.7)
    ...
    if hasattr(state.test, 'SwT') and state.test.SwT:
        ax.set_title("Траектория движения и поверхность", fontsize=9, fontweight="normal")
    else:
